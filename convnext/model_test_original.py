...
"""
def one_shot_test(test_dataset: OneShotImageDataset,model,threshold,use_argmin,quantify_wrong,embedding_size=2048):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for j,k in enumerate(images.keys()):
        if len(images[k]) > 1:
            ...
            rest = torch.Tensor(len(images.keys()),embedding_size).to(device) # allocate space for all cow classes available
            ...
            if use_argmin:
                ...
                if results[j] == 1.0 and results.sum(0) == 1:
                    ...
                    if results[j] == 1.0 and quantify_wrong:
